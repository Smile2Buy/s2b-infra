...
resource "aws_iam_policy" "ec2_fa_policy" {
  ...
  policy = data.aws_iam_policy_document.ec2_fa_policy.json
}

resource "aws_iam_policy" "s3_fa_policy" {
  name        = "s3_fa_policy"
  description = "Allows FullAccess for remote backend management"

  policy = data.aws_iam_policy_document.s3_fa_policy.json
}

resource "aws_iam_policy" "dynamodb_fa_policy" {
  name        = "dynamodb_fa_policy"
  description = "Allows FullAccess for remote backend management"

  policy = data.aws_iam_policy_document.dynamodb_fa_policy.json
}
...
resource "aws_iam_user_policy_attachment" "user_dynamodb-fa-policy_attachment" {
  user       = data.aws_iam_user.current.user_name
  policy_arn = aws_iam_policy.dynamodb_fa_policy.arn
}
